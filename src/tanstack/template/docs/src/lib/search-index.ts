...
/** Kept for parity with the Next.js template; TanStack build does not use Next cache tags. */
...
async function buildSearchEntries(language?: string): Promise<SearchEntry[]> {
...
export function getSearchEntries(language?: string): Promise<SearchEntry[]> {
  return buildSearchEntries(language)
}
